#python redis pipeline benchmark
import pytest
...
def test_pipeline_memoryview(r):
    with r.pipeline() as pipe:
...
def test_pipeline_no_transaction(r):
...
def test_pipeline_no_transaction_watch(r):
    r['a'] = 0
...
def test_pipeline_no_transaction_watch_failure(r):
    r['a'] = 0
...
def test_exec_error_in_response(r):
    """
...
def test_transaction_with_empty_error_command(r):
    """
...
def test_pipeline_with_empty_error_command(r):
    """
    Commands with custom EMPTY_ERROR functionality return their default
    values in the pipeline no matter the raise_on_error preference
    """
...
def test_parse_error_raised_transaction(r):
    with r.pipeline() as pipe:
        pipe.multi()
        # the zrem is invalid because we don't pass any keys to it
        pipe.set('a', 1).zrem('b').set('b', 2)
...
def test_watch_failure_in_empty_transaction(r):
    r['a'] = 1
    r['b'] = 2
...
def test_watch_exec_no_unwatch(r):
    r['a'] = 1
    r['b'] = 2
...
def test_watch_reset_unwatch(r):
    r['a'] = 1
...
def test_transaction_callable(r):
    r['a'] = 1
    r['b'] = 2
    has_run = []
...
def test_transaction_callable_returns_value_from_callable(r):
    def callback(pipe):
...
def test_exec_error_in_no_transaction_pipeline(r):
    r['a'] = 1
    with r.pipeline(transaction=False) as pipe:
        pipe.llen('a')
        pipe.expire('a', 100)
...
def test_exec_error_in_no_transaction_pipeline_unicode_command(r):
...
def test_pipeline_with_bitfield(r):
    with r.pipeline() as pipe:
        pipe.set('a', '1')
        bf = pipe.bitfield('b')
        pipe2 = (bf
